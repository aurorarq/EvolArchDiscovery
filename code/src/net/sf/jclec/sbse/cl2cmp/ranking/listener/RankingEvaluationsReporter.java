...
public class RankingEvaluationsReporter implements IAlgorithmListener, IConfigure {
	 ...
	 * Empty Constructor
	 * */
	public RankingEvaluationsReporter() {
		super();
	}
	...
	protected void doIterationReport(PopulationAlgorithm algorithm, boolean force) {
		...
		if (!force && evaluations%this.reportFrequency != 0) {
			return;
		}	
	...
	protected void createName(PopulationAlgorithm algorithm){
		...
		this.reportTitle += "-e" + ((PopulationAlgorithm)algorithm).getEvaluator().getNumberOfEvaluations();
